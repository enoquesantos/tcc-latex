...
\begin{itemize}
	...
	\item \textit{events}: (objeto): Um objeto que mapeia os identificadores de eventos, baseados em um par de strings \textit{nome-do-evento.valor}. Essa propriedade deverá ser utilizada por objetos de plugins e componentes internos para identificarem de qual evento estão sendo notificados, a fim de padronizar os nomes dos eventos e reduzir a replicação de strings na aplicação. O objeto \textit{App} adiciona quatorze nomes de eventos, alguns são utilizados somente por objetos internos, outros podem ser utilizados pelos plugins para executarem ações específicas em dado momento. Esses eventos é descrito a seguir:
	\begin{itemize}
		\item \textit{cameraImageSaved} (string): Utilizado para notificar que uma imagem capturada pela câmera do dispositivo foi salva localmente. A url da imagem salva será passado no segundo argumento do evento;

		\item \textit{cancelSearch} (string): Utilizado pelo ToolBar quando a busca na página vista pelo usuário é cancelada. Uma página de um plugin poderá permitir que um campo de busca seja utilizado pelo usuário definindo o status do \textit{ToolBar} para "search"

		\item \textit{eulaAccepted} (string): 

		\item \textit{logoutApplication} (string): 

		\item \textit{newIntentPushMessage} (string): 

		\item \textit{newPushMessage} (string): 

		\item \textit{newPushNotificationToken} (string): 

		\item \textit{openDrawer} (string): 

		\item \textit{popCurrentPage} (string): 

		\item \textit{refreshDrawerPages} (string): 

		\item \textit{requestUpdateUserProfile} (string): 

		\item \textit{initUserProfile} (string): 

		\item \textit{setUserProfileData} (string): 

		\item \textit{userProfileChanged"} (string):
	\end{itemize}
\end{itemize}
