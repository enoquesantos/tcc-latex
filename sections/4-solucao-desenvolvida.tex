...
O trecho de código a seguir, apresenta um exemplo de como criar um \textit{Bind} com o status de uma requisição em uma propriedade de um objeto \textit{Button}.
...
A arquitetura dispõe do plugin de exemplo \textit{Listeners} que contém o componente \textit{PushNotificationRegister.qml}. Esse componente demonstra um exemplo de como obter o token de registro no \textit{Firebase} e enviar para o serviço \textit{REST do aplicativo}. O trecho de código a seguir, apresenta um exemplo de como acessar o token do \textit{Firebase}.
