...
O desenvolvedor poderá adicionar os eventos que deseja utilizar entre os plugins no arquivo de configuração, na propriedade \textit{events} e utilizá-los via acesso ao objeto \textit{Config}. Os eventos são basicamente strings e o acesso pode ser feito da seguinte forma: \textit{Config.events.event\_name}. Outro detalhe é que \textit{Subject} criará uma \textit{thread} para cada \textit{observer} quando for notificá-los de algum evento. Isso significa que a chamada ao método \textit{update} em cada \textit{observer} será assíncrono. O Exemplo 12, demonstra como utilizar o \textit{Observer} pelos plugins, realizando uma operação de inserção no banco de dados quando o objeto for notificado.
