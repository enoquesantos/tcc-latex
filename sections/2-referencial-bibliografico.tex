...
Esta seção, apresenta as principais referências que contextualizam este trabalho. A subseção 2.1 descreve sobre Arquitetura de Software. A subseção 2.2 resume Sistemas de Informação. A subseção 2.3 apresenta Arquitetura de Referência para Sistemas de Informação. A subseção 2.4 apresenta de forma geral Arquiteturas de Aplicativos Móveis. A subseção 2.6 faz uma breve revisão sobre Projetos de Aplicativos Móveis. A subseção 2.7 destaca sobre Desenvolvimento de Software Orientado a Componentes. Para finalizar, a subseção 2.8 faz uma breve revisão sobre tecnologias suportadas nesta arquitetura: \textit{web services}, o estilo arquitetural \textit{REST}, o \textit{Push Notification} e o JSON.
